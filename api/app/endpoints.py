...
def create_zone(zone: schemas.DeforestedZoneCreate, db: Session = Depends(database.get_db)):
    return crud.create_deforested_zone(db, zone)
...
def read_zones(skip: int = 0, limit: int = 100, db: Session = Depends(database.get_db)):
    zones = crud.get_deforested_zones(db, skip=skip, limit=limit)
    return zones

@router.get("/zones/{zone_id}", response_model=schemas.DeforestedZone)
def read_zone(zone_id: int, db: Session = Depends(database.get_db)):
    db_zone = crud.get_deforested_zone(db, zone_id)
    if db_zone is None:
...
def update_zone(
    zone_id: int, 
    zone: schemas.DeforestedZoneUpdate, 
    db: Session = Depends(database.get_db)
):
    db_zone = crud.update_deforested_zone(db, zone_id, zone)
    if db_zone is None:
        raise HTTPException(status_code=404, detail="Zone not found")
    return db_zone
...
def delete_zone(zone_id: int, db: Session = Depends(database.get_db)):
    db_zone = crud.delete_deforested_zone(db, zone_id)
    if db_zone is None:
        raise HTTPException(status_code=404, detail="Zone not found")
    return db_zone